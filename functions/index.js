...
const PROMPT = [
  ...
  '  from the label (that goes in "description").',
  '- "description": the fuller descriptive/legal text on the label, e.g.',
  '  "Distiller\'s Select Kentucky Straight Rye Whiskey" — empty string if the',
  '  label has nothing beyond the name',
  '- "producer": the producer / winery / distillery',
  ...
  async (request) => {
    ...
    try {
      ...
      const data = {
        ...
        price: String(w.price == null ? '' : w.price).replace(/[^0-9.]/g, '').trim(),
        predLow: coerceScore(w.predLow),
        predHigh: coerceScore(w.predHigh),
        fit,
        verdict: String(w.verdict || '').trim()
